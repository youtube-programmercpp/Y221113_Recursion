// C言語再帰処理の練習プログラム: #から始まる行をすべて表示 Copyright(C) 2022 https://www.youtube.com/@ProgrammerCpp
...
static void そこまでの文字群を表示する(const struct Node* 履歴)
{
	if (履歴) {
		そこまでの文字群を表示する(履歴->前の文字ノード);
		putchar(履歴->この時点での文字);
	}
}
	...
	for (;;) {
		...
		default:
			if (isspace((unsigned char)行開始文字候補)) {
				...
				return 行開始文字が特定文字である場合に限り表示する(fp, &(struct Node) {(char)行開始文字候補, 履歴});
			}
			else {
...
void 非空白行頭が特定文字である行を全て表示(FILE* fp)
{
	while (行開始文字が特定文字である場合に限り表示する(fp, NULL))
		;
}
